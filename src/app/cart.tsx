...
export default function CartScreen() {
    ...
    return (
        <View className="flex-1 bg-background-dark">
            ...
            <SkiaBlurGlow
                color="rgba(70, 236, 19, 0.2)"
                blur={120}
                size={350}
                offsetY={-100}
            />

            {/* Secondary blue glow (bottom-right) - simulated with View */}
            <View
            ...
            <SafeAreaView className="flex-1" edges={['top']}>
                {/* Header - Glass Panel */}
                <View
                    className="px-4 py-3 flex-row items-center justify-between bg-background-dark/60 border-b border-white/10"
                >
                    <TouchableOpacity
                        onPress={() => { Haptics.selectionAsync(); router.back(); }}
                        className="w-10 h-10 rounded-full items-center justify-center hover:bg-white/10"
                    >
                        <MaterialIcons name="arrow-back-ios-new" size={20} color="white" />
                    </TouchableOpacity>
                    <Text className="text-base font-bold text-white/90">Seu Carrinho</Text>
                    <View className="w-10 items-center">
                        <Text className="text-sm font-semibold text-primary">{items.length}</Text>
                    </View>
                </View>
                ...
                >
                    ...
                    <View className="px-4 gap-4">
                        {items.map((item) => (
                            <View
                                key={item.id}
                                className="p-3 rounded-2xl bg-background-dark/60 border border-white/10"
                                style={{
                                    shadowColor: '#000',
                            ...
                            >
                                <View className="flex-row items-center gap-4">
                                    ...
                                    <View className="flex-1">
                                        ...
                                        <Text className="text-white/60 text-xs">{item.volume}</Text>
                                        <Text className="text-primary font-mono font-medium mt-1">
                                            R$ {item.price.toFixed(2).replace('.', ',')}
                                        </Text>
                                    </View>

                                    {/* Quantity Controls */}
                                    <View className="flex-row items-center bg-black/30 rounded-full p-1 border border-white/5">
                                        <TouchableOpacity
                                        ...
                                        <Text className="w-8 text-center font-mono text-sm font-bold text-white">
                                            {item.quantity}
                                        </Text>
                                        <TouchableOpacity
                                            onPress={() => updateQuantity(item.id, 1)}
                    ...
                    <View className="mt-8">
                        <View className="flex-row items-center gap-2 px-4 mb-4">
                            ...
                            <Text className="text-lg font-bold text-white">Complete seu pedido</Text>
                        </View>
                        ...
                        >
                            {UPSELL_ITEMS.map((item) => (
                                <View
                                    key={item.id}
                                    className="w-[140px] p-3 rounded-2xl mr-4 bg-background-dark/60 border border-white/10"
                                    style={{
                                        shadowColor: '#000',
                                        shadowOffset: { width: 0, height: 8 },
                                        shadowOpacity: 0.2,
                                ...
                                >
                                    ...
                                    <Text className="text-xs text-primary font-mono mb-3">{item.price}</Text>
                                    <TouchableOpacity
                                        onPress={() => Haptics.selectionAsync()}
                                        className="w-full py-2 rounded-full bg-white/10 items-center border border-white/5"
                                    >
                ...
                <View className="relative max-w-lg mx-auto w-full gap-4">
                    ...
                    <View className="gap-1 px-2">
                        <View className="flex-row justify-between">
                            ...
                            <Text className="text-sm font-mono text-white/60">
                                R$ {subtotal.toFixed(2).replace('.', ',')}
                            </Text>
                        </View>
                        <View className="flex-row justify-between">
                            <Text className="text-sm text-white/60">Entrega</Text>
                            <Text className={`text-sm font-mono ${deliveryFee === 0 ? 'text-primary' : 'text-white/60'}`}>
                                {deliveryFee === 0 ? 'GRÁTIS' : `R$ ${deliveryFee.toFixed(2).replace('.', ',')}`}
                            </Text>
                        </View>
                        <View className="flex-row justify-between mt-1">
                            <Text className="text-lg font-bold text-white">Total</Text>
                            <Text className="text-lg font-mono font-bold text-white">
                    ...
                    <TouchableOpacity
                        ...
                        className="relative h-14 rounded-full bg-white/10 overflow-hidden flex-row items-center p-1 border border-white/5"
                        activeOpacity={0.95}
                        style={{
                            shadowColor: '#000',
                    ...
                    >
                        ...
                        <View className="flex-1 items-center pr-4">
                            <Text className="text-white font-bold tracking-wide">Ir para Pagamento</Text>
                        </View>
                    </TouchableOpacity>
                </View>
            </View>
